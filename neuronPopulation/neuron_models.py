...
stdp_inhibitory_weight_max = 40.0 #30
stdp_excitatory_increase_nu = 0.01#0.01
stdp_excitatory_decrease_nu = 0.015#0.0075#0.015
stdp_inhibitory_increase_nu = 0.03#0.03
stdp_inhibitory_decrease_nu = 0.045#0.045
stdp_tau_e = 20.0 #NOTE: In the paper by Juljana on Triplets the tau_- ~= 2*tau_+
stdp_tau_i = 20.0#40.0
triplet_rule_active = False
triplet_modulation_tau = stdp_tau_e*7. #
triplet_stdp_tau_e = stdp_tau_e*2
delete_synapse_threshold = 0.0 #prunning

#parameters for other rules
weight_decay_e = 0.#0.1*stdp_excitatory_increase_nu# oja-style weight decay
weight_decay_i = 0.#0.2*stdp_inhibitory_increase_nu
inhibitory_kernel = 'c'
...
class Synapse:
    ...
    def stdp(self, Dt_post = 0.):
        if self.weight != irresistible_spike and self.stdp_active:
            ...
            if presynaptic_time != non_existant_time and postsynaptic_time != non_existant_time:
                ...
                Dw = self.compute_weight_change(presynaptic_time, postsynaptic_time, Dt_post, inhibitory_kernel)
                self.change_weight( Dw)
                
        if abs(self.weight) < delete_synapse_threshold:
            self.invalidate_synapse()
    
    def compute_weight_change(self, presyn_time, postsyn_time, Dt_post = 0., kernel_I = 'c'):
        stdp_reinforces = presyn_time < postsyn_time
        A = self.A_stdp(stdp_reinforces)
        stdp_decay = self.stdp_decay(postsyn_time - presyn_time)#  np.exp(-abs(postsynaptic_time - presynaptic_time)/stdp_tau )
        
        if self.excitatory:
            Dw = A*stdp_decay
            if triplet_rule_active:
                Dw = Dw*np.exp(-abs(Dt_post)/triplet_modulation_tau)
        else:
            if kernel_I == 'c': #Classical STDP
                Dw = A*stdp_decay
            if kernel_I == 'r': #reversed STDP
                Dw = -A*stdp_decay
            if kernel_I == 'p': # all postitive STDP
                Dw = abs(A*stdp_decay)
            if kernel_I == 'n': # all negative STDP
                Dw = -abs(A*stdp_decay)
            if kernel_I == 's': # sombrero
                Dw = self.sombrero_kernel(postsyn_time - presyn_time)
                
        return Dw

    def sombrero_kernel(self,Dt):
        d = 2*np.exp(-(2*Dt/stdp_tau_i)**2) - 0.66*np.exp(-(Dt/stdp_tau_i)**2)
        softCoeff = (stdp_excitatory_weight_max - abs(self.weight))
        Dw = d*stdp_inhibitory_increase_nu*softCoeff
        return Dw
    ...
    def stdp_noise(self, delta_t):
        if self.stdp_active:
            ...
            A = self.A_stdp(stdp_reinforces)
            stdp_decay = self.stdp_decay(delta_t) #np.exp(-abs(delta_t/stdp_tau) )
            self.change_weight( A*stdp_decay)
    
    def stdp_decay(self,delta_t):
        if self.excitatory:
            return np.exp(-abs(delta_t)/stdp_tau_e)
        else:
            return np.exp(-abs(delta_t)/stdp_tau_i)
        
    def weight_decay(self, rate):
        if self.excitatory:
            self.change_weight(-self.weight*weight_decay_e*rate)
        else:
            self.change_weight(-self.weight*weight_decay_i*rate)
    ...
    def A_stdp(self, stdp_reinforce):
        ...
        if stdp_reinforce:
            if self.excitatory:    
                ...
                A = softCoeff*stdp_inhibitory_increase_nu
                return -A
        else:
            if self.excitatory:    
...
class Neuron_LIF:
    ...
    def fire(self, time):
        self.membrane_potential = resting_potential
        Dt_triplet = time - self.last_firing
        self.last_spike = time #+ refractory_period
        self.last_firing = time
        self.stdp_on_firing(Dt_triplet)
    ...
    def stdp_on_firing(self, Dt_triplet = 0.):
